...
int main(int argc, char** argv) {
  ...
  if(argc == 1) {
    perror("papicc requires --filename and atleast 1 --performancecounter");
    exit(1);
  }

  for(int i = 0; i < argc, i++) {
    if(strcmp(argv[i], filenameopt[0].c_str(), filenameopt[0].size()) == 0) {
      filename = argv[i+1];
    }
    else if(strcmp(argv[i], filenameopt[1].c_str(), filenameopt[1].size()) == 0) {
      filename = argv[i+1];
    }
  ...
  papi_exec("cc", classname.c_str(), perfcounter);
  return 1;
}
